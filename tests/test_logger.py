...
def test_log_command_marks_stage6_validation_failure_as_error(tmp_path, monkeypatch):
...
def test_log_command_keeps_successful_stage6_command_as_ok(tmp_path, monkeypatch):
    log_path = tmp_path / "commands.log"
    monkeypatch.setattr(logger.os.path, "abspath", lambda _: str(log_path))
